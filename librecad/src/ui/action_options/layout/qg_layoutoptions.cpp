...
/*
 *  Constructs a QG_LayoutOptions as a child of 'parent', with the
 *  name 'name' and widget flags set to 'f'.
 */
QG_LayoutOptions::QG_LayoutOptions()
    :LC_ActionOptionsWidgetBase(RS2::ActionFileLayoutView, "LayoutView", "")
    , m_defaultScalesStartIndex{0}
    , ui(new Ui::Ui_LayoutOptions{})
{
    ui->setupUi(this);

    // Connect ui actions
    connect(ui->cFixed, &QCheckBox::clicked, this, &QG_LayoutOptions::onScaleFixedClicked);
    connect(ui->bScaleLineWidth, &QPushButton::toggled, this, &QG_LayoutOptions::onScaleLineClicked);
    connect(ui->bBlackWhite, &QPushButton::toggled, this, &QG_LayoutOptions::onBlackWhiteClicked);
    ...
    connect(ui->bFit, &QPushButton::clicked, this, &QG_LayoutOptions::onFitClicked);
    connect(ui->bCenter, &QPushButton::clicked, this, &QG_LayoutOptions::onCenterClicked);
    connect(ui->bCalcPagesNum, &QPushButton::clicked, this, &QG_LayoutOptions::onCalcPagesNumClicked);
    connect(ui->bZoomPage, &QToolButton::clicked, this, &QG_LayoutOptions::onZoomToPageClicked);
    connect(ui->cbTiledPrint, &QToolButton::clicked, this, &QG_LayoutOptions::onTiledPrintClicked);

    connect(ui->tbSettings, &QToolButton::clicked, this, &QG_LayoutOptions::onSettingsClicked);
    connect(ui->tbPortait, &QToolButton::clicked, this, &QG_LayoutOptions::onPortraitClicked);
    connect(ui->tbLandscape, &QToolButton::clicked, this, &QG_LayoutOptions::onLandscapeClicked);

    connect(ui->sbPagesVertical, &QSpinBox::valueChanged, this, &QG_LayoutOptions::onVerticalPagesValueChanges);
    connect(ui->sbPagessHorizontal, &QSpinBox::valueChanged, this, &QG_LayoutOptions::onHorizontalPagesValueChanges);
 ...
 */
QG_LayoutOptions::~QG_LayoutOptions() = default;

/*
 *  Sets the strings of the subwidgets using the current
 *  language.
 */
void QG_LayoutOptions::languageChange(){
    ui->retranslateUi(this);
}

void QG_LayoutOptions::doSaveSettings() {
...
void QG_LayoutOptions::doSetAction(RS_ActionInterface *a, bool update) {
    m_action = dynamic_cast<RS_ActionLayoutView*>(a);
...
QStringList QG_LayoutOptions::readCustomRatios(bool metric) {
    QStringList ratios;
...
void QG_LayoutOptions::saveCustomRatios() {
...
void QG_LayoutOptions::initializeScaleBoxItems() {
...
void QG_LayoutOptions::addScalesToCombobox(QStringList &scales){
...
bool QG_LayoutOptions::addScaleToScaleCombobox(const QString& scaleString){
    bool parseOk;
...
void QG_LayoutOptions::onCenterClicked() {
   m_action->center();
}

void QG_LayoutOptions::onZoomToPageClicked() {
    m_action->zoomToPage();
}

void QG_LayoutOptions::onTiledPrintClicked(){
...
void QG_LayoutOptions::onSettingsClicked(){
    m_action->invokeSettingsDialog();
}

void QG_LayoutOptions::onPortraitClicked(){
    bool portrait = ui->tbPortait->isChecked();
    setPaperOrientation(portrait);
    m_action->setPaperOrientation(portrait);
}

void QG_LayoutOptions::onLandscapeClicked(){
    bool portrait = !ui->tbLandscape->isChecked();
    setPaperOrientation(portrait);
    m_action->setPaperOrientation(portrait);
}

void QG_LayoutOptions::setPaperOrientation(bool isPortait){
...
void QG_LayoutOptions::onVerticalPagesValueChanges(int value) {
    m_action->setPagesNumVertical(value);
}

void QG_LayoutOptions::onHorizontalPagesValueChanges(int value) {
    m_action->setPagesNumHorizontal(value);
}

void QG_LayoutOptions::onScaleLineClicked(bool state) {
    setScaleLineToUI(state);
    m_action->setLineWidthScaling(state);
}

void QG_LayoutOptions::setScaleLineToUI(bool state) {
    if(ui->bScaleLineWidth->isChecked() != state) {
        ui->bScaleLineWidth->setChecked(state);
    }
}

void QG_LayoutOptions::onBlackWhiteClicked(bool on) {
...
void QG_LayoutOptions::onFitClicked() {
    if (!ui->cFixed->isChecked()) {
        fitPage();
    }
}

void QG_LayoutOptions::fitPage() {
    m_action->fit();
    updateScaleBox(m_action->getScale());
}

/** print scale fixed to saved value **/
void QG_LayoutOptions::onScaleFixedClicked(bool fixed){
    m_action->setPaperScaleFixed(fixed);
    setScaleFixedToUI(fixed);
}

void QG_LayoutOptions::setScaleFixedToUI(bool fixed) {
...
void QG_LayoutOptions::onCalcPagesNumClicked() {
    if (m_action != nullptr) {
        m_action->calcPagesNum(true);
    }
}

void QG_LayoutOptions::scale(const QString &newScale, bool force) {
    QString scaleToUse;
    if (force){
        scaleToUse = newScale;
    }
...
double QG_LayoutOptions::parseScaleString(const QString &scaleText, bool &parseOk) const {
    double factor = 1.0;
    parseOk = false;
...
void QG_LayoutOptions::updateScaleBox(double factor){
    //    std::cout<<"void QG_LayoutOptions::updateScaleBox() f="<<f<<std::endl;
    int scaleIndex;
    int existingScalesCount = ui->cbScale->count();
...
void QG_LayoutOptions::updatePageNumbers() {
